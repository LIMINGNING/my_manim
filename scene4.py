...
class scene4(FrisbeeBaseScene):
    def construct(self):
        ...
        arr1_label=MathTex("Fake", font_size=32).next_to(arr1, RIGHT)
        self.play(Create(arr1))
        self.play(Write(arr1_label))
        self.play(FadeOut(arr1_label),FadeOut(arr1))

        self.move_multiple_players([
            (attackers[6], np.array([1.5,1,0]), False),  # 第1个玩家
            (defender[6], np.array([1,1.5,0]), False)    # 第2个玩家
        ],
        run_time=1.5,
        path_type="straight",
        ...
        arr2_label=MathTex("Go\ deep", font_size=32).next_to(arr2, RIGHT)
        huck_label=MathTex("Huck", font_size=32).next_to(frisbee, RIGHT)
        self.play(Create(arr2))
        self.play(Write(arr2_label),Write(huck_label))
        self.play(FadeOut(arr2_label),FadeOut(arr2),FadeOut(huck_label))
        ...
        self.fly_frisbee(
            ...
            direction=RIGHT,
            defender=defender[6],
            target_player_movement=np.array([3,6.5,0]),
            defender_movement=np.array([2.5,6,0]),
            is_camera_move=True,
            vertical_only=True,
            flight_type="right",
            is_relative=False,
